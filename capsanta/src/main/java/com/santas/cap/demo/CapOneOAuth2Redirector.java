...
class CapOneOAuth2Redirector {
    ...
    private Supplier<String> cypherSupplier = () ->
            Arrays.asList("TOTH", "IBIS", "KEY", "TIME", "ETC")
...
class CapOneOauth2ClientConfig {
    ...
    CapOneAuthAccountDetails capOneAuthDetails() {
    ...
    @Bean
    OAuth2RestOperations restTemplate(@Qualifier("capOne") OAuth2ClientContext authContext,
                                      CapOneAuthResourceDetails authRes) {
        return new OAuth2RestTemplate(authRes, authContext);
    }
